...
s3_prefix <- Sys.getenv("S3_PREFIX", unset = "fsa-counties-dd17")
...
mapshaper \\
  ...
  -o format=topojson quantization=1e6 fix-geometry id-field='id' bbox target=* fsa-counties-dd17.topojson
"
)
...
s3_put(bucket = s3_bucket,
       key = paste0(s3_prefix, "/FSA_Counties_dd17.gdb.zip"),
       file = "FSA_Counties_dd17.gdb.zip",
       content_type = "application/zip")

s3_put(bucket = s3_bucket,
       key = paste0(s3_prefix, "/fsa-counties-dd17.topojson"),
       file = "fsa-counties-dd17.topojson")

s3_put(bucket = s3_bucket,
       key = paste0(s3_prefix, "/fsa-counties-dd17.parquet"),
       file = "fsa-counties-dd17.parquet",
...
cf_invalidate(
  paths = c(
    paste0("/", s3_prefix, "/FSA_Counties_dd17.gdb.zip"),
    paste0("/", s3_prefix, "/fsa-counties-dd17.topojson"),
    paste0("/", s3_prefix, "/fsa-counties-dd17.parquet"),
